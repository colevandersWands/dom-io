...
whenFormDataChanges('user-data', () => {
  ...
  // --- error case: reading an unsupported input type
  // readString('pw'); // input password
  const pig = readString('pig'); // select options
  const toad = readString('toad'); // fieldset input radio

  // --- reading booleans ---
  const red = readBoolean('red');

  // --- reading numbers ---
  const x = readNumber('x'); // range
  const y = readNumber('y'); // number
